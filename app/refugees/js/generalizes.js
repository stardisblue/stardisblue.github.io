...
function regresarTiempo(polaridad,step){
	
	console.log("polaridad",polaridad, "step",step)
	
	var result;
	switch(polaridad.toLowerCase()){
		case "m":
			result = d3.time.minutes,step;
			break;
		case "h":
			result = d3.time.hours,step;
			break;
		case "d":
			result = d3.time.days,step;
			break;
		case "b":
			result = d3.time.months,step;
			break;
		case "y":
			result = d3.time.years,step;
			break;
		default:
			console.log("Problem regresarTiempo")
			break;
...
//arg is an array like [1,2,3,4,5,6]
function sum(arg){
	return result = arg.reduce((accumulate, current)=>{
...
function calculeNumIntervals(brushObject, polarityTemporal, stepTemporal) {
	...
	case "d":result = d3.time.days(brushObject.extent()[0], brushObject.extent()[1],stepTemporal).length;
		break;
//	case 3:result = d3.time.weeks(brushObject.extent()[0],brushObject.extent()[1], stepTemporal).length;
//		break;
...
function getNumIntervalsDistortion(polarityTemporal, i, j){
...
* @returns
*/
//function getDataProcessing(stepTemporal) {
//	console.log("getDataProcessing........")
//	// DateRange : get a new date dataset with the MinRange and MaxRange likelimites EVERY stepTemporal time
//	// dateRange = d3.time.minutes(d3.time.minute.floor(dateMinRange),d3.time.minute.ceil(dateMaxRange),stepTemporal);
//	switch (polarityTemporal) {
//		case 0: dateRange = d3.time.minutes(d3.time.minute.floor(dateMinRange),d3.time.minute.ceil(dateMaxRange), stepTemporal);
//			break;
//		case 1: dateRange = d3.time.hours(d3.time.hour.floor(dateMinRange),d3.time.hour.ceil(dateMaxRange), stepTemporal);
//			break;
//		case 2: dateRange = d3.time.days(d3.time.day.floor(dateMinRange), d3.time.day.ceil(dateMaxRange), stepTemporal);
//			break;
//		case 3: dateRange = d3.time.weeks(d3.time.week.floor(dateMinRange),d3.time.week.ceil(dateMaxRange), stepTemporal);
//			break;
//		case 4: dateRange = d3.time.months(d3.time.month.floor(dateMinRange),d3.time.month.ceil(dateMaxRange), stepTemporal);
//			break;
//		case 5: dateRange = d3.time.years(d3.time.year.floor(dateMinRange),d3.time.year.ceil(dateMaxRange), stepTemporal);
//			break;
//	}
//
...
////						.call(xAxisContext);
//
////	d3.select("#stepTemporal-text").text(stepTemporal);
////	d3.select("#stepTemporal-value").property("value", stepTemporal);
//
