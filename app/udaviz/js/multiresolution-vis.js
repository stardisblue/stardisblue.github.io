var opts = {
	...
	//
	minSizeTextLabel : 25, // 15,//Minimum label size of text in label flow
	maxSizeTextLabel : 80, // 35,//Maximum label size of text in label flow
	//
	//
	flowLabel : "flowLabelImg", //flowLabelText or flowLabelImg
	//
...
function grouped(){
	...
	let neste = (nest_by_key.entries(aryPointDetection));
	
	let gGroupPointByKey = focus.select("#gPointChanges").selectAll(".point-detection")
										.data(neste,function(d){return d.key;});

	gGroupPointByKey.exit().remove();

	gGroupPointByKey.enter().append("g").attr("class",function(d){return "point-detection " + d.key;});

	// let pointValues = gGroupPointByKey.selectAll("circle")

	// //exit
	// gGroupPointByKey.exit().remove();

	// gGroupPointByKey.enter().append("g").attr("class","point-detection " +layerInZoom.key);



	// let points = gGroupPointByKey.select("#gPointChanges").selectAll(".point-detection."+layerInZoom.key)
...
function getFlowLabelWithoutOverlapping(){
...
function flowTextLabel(){
	let flowWithoutOverlapping = getFlowLabelWithoutOverlapping();
	let flowLabel = focus.select("#textsLabels").selectAll(".textLabel")
...
//						return d.date;
					...
					.y1(function(d) {return yScaleFocus(d.y0 + d.y + hola);});
